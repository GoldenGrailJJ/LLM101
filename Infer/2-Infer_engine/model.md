...
| 参数类型           | 是否需要保存 | 原因                                                                                         |
|--------------------|--------------|----------------------------------------------------------------------------------------------|
| 模型权重           | ✅ 是         | 权重是模型的核心，包含了网络层的参数，用于计算推理结果，是模型推理的基础，必须保存。           |
| 模型元信息         | ✅ 是         | 包含模型架构的超参数（如层数、维度、头数等），推理时需要这些信息来正确组织权重和分配资源。     |
| 量化信息           | ✅ 是         | 量化后的模型需要反量化以恢复权重的浮点值，缩放因子和零点等信息是正确推理的必要条件。           |
| 位置编码           | ✅ 是         | 自注意力机制中需要位置信息，如果动态生成成本高或不方便，提前保存可提高推理效率。                |
| 优化器状态         | ❌ 否         | 优化器状态（如动量、二阶矩）仅用于训练阶段的参数更新，推理时无用，保存会浪费存储空间。         |
| 梯度               | ❌ 否         | 梯度用于反向传播更新权重，推理时不涉及反向传播，因此无需保存梯度信息，减少内存占用。            |
| 中间激活值         | ❌ 否         | 中间激活值在推理时动态生成，不需要预先存储，保存会导致文件体积增加，且推理中无必要性。          |
| Key-Value 缓存     | ❌ 否         | Key 和 Value 缓存在每次推理中动态生成，序列之间的缓存不同，保存会增加复杂性且无法泛化。         |
| 损失函数相关参数   | ❌ 否         | 损失函数及其相关参数仅在训练时使用，推理时不涉及损失计算，保存无意义且会浪费存储资源。          |
